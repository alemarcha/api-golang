...
func main() {
	...
	datas = session.DB("app").C("data")

	// Set up routes
	r := mux.NewRouter()
	r.HandleFunc("/data", createData).
		Methods("POST")
	r.HandleFunc("/data", readData).
		Methods("GET")
	...
	log.Println("Listenisng dson port 8080...")
}
...
func readData(w http.ResponseWriter, r *http.Request) {
	log.Println("llegando")
	var result []interface{}
	if err := datas.Find(nil).All(&result); err != nil {
		responseError(w, err.Error(), http.StatusInternalServerError)
	} else {
		responseJSON(w, result)
	}
